...
@objc public protocol DHCycleTimerViewDelegate: NSObjectProtocol {
	...
	func cycleTimerViewTimeout(view: DHCycleTimerView)
	
	func cycleTimerView(view: DHCycleTimerView, tick: Int)
}
	...
	private func startTimer(reset: Bool) {
		...
		timer!.setEventHandler {
			...
			if self.isStarted, self.currentTime >= self.maxTime {
				self.timeoutProcess()
				self.delegate?.cycleTimerViewTimeout(view: self)
				return
			}
			...
			if Int(self.millisecondsCount) % 1000 == 0 {
				self.delegate?.cycleTimerView(view: self, tick: self.currentTime)
			}
		}
