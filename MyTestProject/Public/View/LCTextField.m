...
@interface LCTextField () {
	...
	unsigned int            strLengthCustom; //自定义输入长度
...
@implementation LCTextField
	@synthesize /*isMenuItemsEnable, */isRuled, disableEmoji, disableSpace, disableSpecialChar, disableUnderRegEx, isChecking, checkOnResign, checkOnTextChanged, chineseStrLength;
...
- (void)textFieldChanged:(NSNotification *)notification {
	...
	if (!disableEmoji) {
		[LCTextField disableEmojiInString:self.text];
	}
	if (disableSpace) {
		[LCTextField disableSpaceInString:self.text];
	}
	if (disableSpecialChar) {
		[LCTextField disableSpecialCharInString:self.text];
	}
	//获取过滤后的内容
	NSString *result = [NSString stringWithFormat:@"%@", self.text];
	...
	if (!position /*|| !selectedRange*/) {
		...
		for (unsigned int i = 0; i < result.length; i++) {
			...
			if (strLengthCustom != 0) {
				if (strLength > strLengthCustom) {
...
- (BOOL)canPerformAction:(SEL)action withSender:(id)sender
	{
		//安全输入模式不能剪切、复制【直接不显示】
		if (self.secureTextEntry || self.isMenuHidden)
		{
